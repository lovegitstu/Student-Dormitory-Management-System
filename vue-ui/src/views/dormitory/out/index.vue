  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="" prop="fId" v-hasRole="['admin', 'subadmin']">
        <el-select v-model="queryParams.fId" placeholder="请选择楼栋" clearable>
      ...
      <el-form-item label="" prop="datetime">
        <el-date-picker clearable v-model="queryParams.toDatetime" type="date" value-format="yyyy-MM-dd"
          placeholder="请选择离校时间">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="" prop="approvalStatus" v-hasRole="['admin', 'subadmin']">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['dormitory:out:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['dormitory:out:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['dormitory:out:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['dormitory:out:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table border v-loading="loading" :data="outList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="离校原因" align="center" prop="excuse" />
      <el-table-column label="离校时间" align="center" prop="toDatetime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.toDatetime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="400">
        <template slot-scope="scope">
          ...
          <el-button 
            ...
            v-hasPermi="['dormitory:out:approve']">通过</el-button>
          <el-button 
            v-if="scope.row.approvalStatus === '0'" 
            size="small" 
            type="warning" 
    ...
    <el-dialog :title="title" :visible.sync="open" width="340px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="宿舍楼层" prop="fId">
          <el-select v-model="form.fId" placeholder="请选择宿舍楼" @change="parentSelect('addEditSelect')" :disabled="currentRole === 'student'">
        ...
        <el-form-item label="宿舍编号" prop="dorId">
          <el-select v-model="form.dorId" placeholder="请选择宿舍" @change="childSelect('addEditSelect')" :disabled="currentRole === 'student'">
        ...
        <el-form-item label="学生姓名" prop="stuName">
          <el-input v-model="form.stuName" placeholder="请输入学生姓名" :disabled="currentRole === 'student'" />
        </el-form-item>
        <el-form-item label="离校原因" prop="excuse">
          <el-input v-model="form.excuse" placeholder="请输入离校原因" />
        </el-form-item>
        <el-form-item label="离校时间" prop="datetime">
          <el-date-picker clearable v-model="form.toDatetime" type="date" value-format="yyyy-MM-dd" placeholder="请选择离校时间">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="审批状态" prop="approvalStatus" v-if="currentRole === 'sysadmin' || currentRole === 'teacher'">
...
export default {
  name: "Out",
  dicts: ['student_status', 'approval_status'],
  data () {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        excuse: [
          { required: true, message: "离校原因不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    getList () {
      ...
      listOut(this.queryParams).then(response => {
        ...
        // 添加调试信息
        console.log('离校申请列表数据:', this.outList);
    ...
    handleDelete (row) {
      ...
      this.$modal.confirm('是否确认删除离校登记？').then(function () {
        return delOut(ids);
      }).then(() => {
        this.getList();
    ...
    handleApprove(row, status) {
      console.log('离校申请审批按钮点击 - 行数据:', row);
      console.log('审批状态:', status);
      console.log('当前用户角色:', this.currentRole);
      console.log('用户权限:', this.$store.getters.permissions);
      
      const statusText = status === '1' ? '通过' : '拒绝';
      this.$modal.confirm(`是否确认${statusText}该离校申请？`).then(() => {
        const data = {
          id: row.id,
          approvalStatus: status
        };
        return this.approveOut(data);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport () {
      this.download('dormitory/out/export', {
        ...this.queryParams
